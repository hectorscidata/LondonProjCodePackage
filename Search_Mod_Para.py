...
#df = pp.build_training_data_set(fire_only=True, max_num_call=2, pump_order_max=2, minturnout=10, mintravel=10,
#                            minattendance=30, max_speed=90, max_dist=12, min_dist=0.3, drop_grenfell=True,
#                            maxturnout=180, frm_date='2015-12-31')
df = pd.read_csv("clean_fire_data_set3.csv", sep=';', index_col=0)
df.info()
...
try:
    para_reg = joblib.load(f'para_{reg}_reg_{s.y_var}.pkl')
    mod_reg = joblib.load(f'mod_{reg}_reg_{s.y_var}.pkl')
except:
    ...
    joblib.dump(para_reg, f'para_{reg}_reg_{s.y_var}.pkl')
    joblib.dump(mod_reg, f'mod_{reg}_reg_{s.y_var}.pkl')

try:
    para_classif = joblib.load(f'para_{classif}_classif_{s.y_var}.pkl')
    mod_classif = joblib.load(f'mod_{classif}_classif_{s.y_var}.pkl')
except:
    ...
    joblib.dump(para_classif, f'para_{classif}_classif_{s.y_var}.pkl')
    joblib.dump(mod_classif, f'mod_{classif}_classif_{s.y_var}.pkl')
